...
- [x] Add configuration ImGUI panels for each family, extended optionally by each system core, to allow for live adjustment of various properties/quirks.
- [x] Extend/refactor the video backend class to support multiple concurrent platform windows and gui sessions that operate independently.
...
- [ ] Design standard ImGUI widgets to recycle for various uses, particularly in debugging tools, such as wave/pattern visualizers, memory viewers, disasm viewers, etc.
