...
public:
	...
	decltype(auto) read(Fn&& function) const noexcept(
		std::is_nothrow_invocable_v<Fn, BufferView<true>> &&
		std::is_nothrow_invocable_v<Fn, BufferView<false>>
	) {
		std::unique_lock lock(mReadLock);

		if (getFlag(mpSwap.load(mo::acquire))) {
			mpRead.store(subFlag(mpSwap.exchange(
				mpRead, mo::acq_rel)), mo::release);

			return std::forward<Fn>(function)
				(BufferView<true>{ *mpRead.load(mo::acquire) });
		} else {
			return std::forward<Fn>(function)
				(BufferView<false>{ *mpRead.load(mo::acquire) });
		}
	}
	 ...
	 * target.
	 *
	 * Callers are recommended to accept the argument as explicitly as auto&.
	 *
	 * @tparam Fn  A callable that can be invoked with auto& and may return
	 *             any type (including void).
	 *
	...
	decltype(auto) write(Fn&& function) noexcept(
		std::is_nothrow_invocable_v<Fn, Buffer&>
	) {
		std::unique_lock lock(mWorkLock);

		if constexpr (std::is_void_v<std::invoke_result_t<Fn, Buffer&>>) {
			std::forward<Fn>(function)(*mpWork);
			mpWork = subFlag(mpSwap.exchange(addFlag(mpWork), mo::acq_rel));
		}
		else {
			decltype(auto) result = std::forward<Fn>(function)(*mpWork);
			mpWork = subFlag(mpSwap.exchange(addFlag(mpWork), mo::acq_rel));
			return result;
		}
	}
