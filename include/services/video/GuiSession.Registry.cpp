...
GuiSession::Handle& GuiSession::attach(
	...
	if (window_handle.is_inert()) {
		blog.warn("The PlatformWindow chosen for attachment with key '{}' is "
			...
			"any rendering/events whatsoever!", cur_key);
	}
	...
	if (s_ctx_props.home_path && s_ctx_props.home_path[0] == '\0') {
		blog.warn("A GuiSession was attached to PlatformWindow with key '{}' "
			...
			"'nullptr' parameter to disable storage, or provide a valid path!", cur_key);
	}
	...
	return internal::s_gui_session_registry.try_emplace(cur_key,
		window_handle, aggregate, font_atlas).first->second;
}
...
GuiSession::Handle* GuiSession::get_main() noexcept {
	auto* main_handle = PlatformWindow::get_main();
	if (!main_handle) { return nullptr; }

	auto* owner_ptr = main_handle->internal_get_owner();
	if (!owner_ptr) { return nullptr; }

	return static_cast<Handle*>(owner_ptr);
}

GuiSession::Handle* GuiSession::get_sync() noexcept {
	auto* live_handle = PlatformWindow::get_sync();
	if (!live_handle) { return nullptr; }

	auto* owner_ptr = live_handle->internal_get_owner();
	if (!owner_ptr) { return nullptr; }

	return static_cast<Handle*>(owner_ptr);
}

GuiSession::Handle* GuiSession::get_live() noexcept {
	auto* live_handle = PlatformWindow::get_live();
	if (!live_handle) { return nullptr; }
...
public:
	SharedContextProperties() noexcept {
		...
		main_font = font_atlas.AddFontFromMemoryCompressedTTF(
...
void GuiSession::Handle::init_context() noexcept {
	...
	io.FontDefault = s_ctx_props.main_font;
...
unsigned UserInterface::get_live_dockspace_id() noexcept {
	auto* live_window = GuiSession::get_live();
